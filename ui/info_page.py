...
_RELEASE_NOTES_TITLE = "Что нового в Beta 1.2.0"

# Левая колонка: исправления
_FIXES = [
    "Исправлена поддержка серверов с XTLS-Vision (flow)",
    "Добавлен Xray как fallback-движок для XHTTP/SplitHTTP",
    "Инсталлер: чистая переустановка без конфликтов",
    "Удалены неиспользуемые файлы из поставки",
    "Обновлена информационная панель приложения",
]

# Правая колонка: улучшения
_IMPROVEMENTS = [
    "Гибридный движок: sing-box + Xray",
    "Расширена поддержка транспортов: XHTTP, SplitHTTP",
    "Парсер: добавлены поля flow, mode, alpn",
    "Автовыбор движка по типу транспорта",
    "Xray 26.2.6 с поддержкой wintun",
]
